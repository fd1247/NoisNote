...
def load_audio_numpy(audio_path, sample_rate=24000, start_second=None, duration=None):
    """使用 soundfile 读取音频；如需重采样或转单声道则回退到 ffmpeg（numpy 重采样对
    大规模数据太慢，而 ffmpeg 的 C 实现可以瞬间完成）。"""
    info = sf.info(audio_path)
    sr = info.samplerate
    channels = info.channels

    # 如果需要重采样或转单声道，直接用 ffmpeg 处理
    if sr != sample_rate or channels > 1:
        return load_audio_ffmpeg(audio_path, sample_rate, start_second, duration)

    # 采样率和声道已匹配，直接用 soundfile 快读
    start_frame = int(start_second * sr) if start_second else 0
    frames = int(duration * sr) if duration else -1

    audio, _sr = sf.read(audio_path, start=start_frame, frames=frames, dtype='float32')
