...
from .schema import MsgType, StreamingMessage, DecodeResult, ASREngineConfig, TranscribeResult, ForcedAlignItem, ForcedAlignResult, ASRProgress
    ...
    def asr(
        ...
        if progress_callback:
            progress_callback(ASRProgress(
                stage="transcribing", current_chunk=0, total_chunks=num_chunks,
                processed_seconds=0.0, total_seconds=total_duration,
                message="正在转录音频",
            ))
        ...
        for i in range(num_chunks):
            ...
            if progress_callback:
                completed = i + 1
                processed = min(completed * chunk_size_sec, total_duration)
                progress_callback(ASRProgress(
                    stage="transcribing",
                    current_chunk=completed, total_chunks=num_chunks,
                    processed_seconds=processed, total_seconds=total_duration,
                    message=f"正在转录音频 {completed}/{num_chunks}",
                ))
