...
```text
创建录音或导入音视频 -> 保存历史记录 -> 任务队列串行处理 -> ASR 转录（含逐句时间轴） -> 可选 LLM 总结 -> 音频回放 / 导出 -> 查看结果
```
...
src/
  ...
  tasks/                                # 任务队列领域模型与持久化
    types.py                            # AppTask、任务状态与阶段
    manager.py                          # TaskManager（串行调度、排序、重试、取消）
    persistence.py                      # TaskQueueStore（task_queue.json）
  ...
  handlers/                             # MainWindow Mixin（每个文件对应一个功能域）
    ...
    summary.py                          # LLM 总结
    task_queue.py                       # 任务队列与现有 worker 的生命周期衔接
  ...
  ui/                                   # Qt 界面组件
    ...
    detail/                             # 历史详情 WebView
      webview.py                        # DetailWebView 外壳、Qt WebChannel 桥接、降级 QTextBrowser
      models.py                         # 详情页 payload/命令校验模型
      metadata.py                       # 详情页元数据格式化
      assets/                           # 本地 HTML/CSS/JS 资源和 Markdown vendor 插件
    dialogs/                            # 业务对话框
      notebook.py                       # 笔记本管理对话框
      recording.py                      # 录音保存/确认对话框
      settings.py                       # 设置相关对话框
    pages/                              # 主界面页面/区域构建
      sidebar.py                        # 侧边栏构建（主侧栏 + 设置侧栏）
      recording.py                      # 录音页面
      content.py                        # 历史详情页壳层、回放栏、标签切换、导出菜单
      result.py                         # 结果状态辅助（转录/总结文本设置、标签切换）
      settings.py                       # 设置面板（通用/模型/热词/快捷键）
      workbench.py                      # 工作区工具栏、任务面板和菜单
    panels/                             # 复合子面板
      model.py                          # 模型管理子页面（树形模型浏览器）
    widgets/                            # 可复用组件
      history_item.py                   # 历史记录列表项（ElidedLabel + 右键菜单）
      history_tree.py                   # 历史记录树/分组列表
...
MainWindow 通过 Python 多重继承组装功能，每个 Mixin 对应 `handlers/` 目录下的一个文件。当前共 12 个 Mixin：
...
| TaskQueueHandlers | task_queue.py | 串行任务调度、任务面板、取消/重试、队列持久化 |
...
`TaskManager` 负责录音、记录处理和远程链接导入任务的状态；其中记录处理与链接导入按 FIFO 串行执行，等待队列上限为 20 项。任务面板将任务分为处理中、排队中和已处理：排队任务可拖拽调整顺序，运行中任务需要协作式取消，已处理任务可重试或移除。

- `TaskQueueStore` 将可恢复的记录处理和远程导入任务写入 `%APPDATA%/NoisNote/task_queue.json`；应用重启时，未完成任务会标记为“应用退出，任务中断”并移至已处理列表，不会自动继续执行。
- 同一记录在运行中或排队中重复提交时复用既有任务；若任务仍在排队，会合并新请求的处理选项，避免重复转录。
- 模型目录、模型文件、GGUF 工具目录、运行时依赖或模型配置等系统性转录错误会暂停后续队列。仅在仍有排队任务时显示“恢复队列”操作；修复环境后由用户手动恢复。
- 队列中的仅总结任务重试时必须保留 `summary_only` 与 `overwrite_existing` 选项，直接从总结阶段继续，不能重新转录。
...
历史详情正文由 `src/ui/detail/webview.py` 承载，优先使用 `QWebEngineView` 加载
`src/ui/detail/assets/index.html`，通过 Qt WebChannel 与 Python 侧通信。WebEngine 不可用
或初始化失败时自动降级到 `QTextBrowser` / `QPlainTextEdit`，保证转录、总结和时间轴内容仍可查看。

- 本地静态资源位于 `src/ui/detail/assets/`，打包时由 `scripts/build.spec` 收集到
  `_internal/src/ui/detail/assets/`。
- `assets/vendor/` 存放第三方 Markdown 渲染库和插件，供详情页离线加载；新增或升级 vendor
  文件时需同步更新该目录 README 中的来源、版本和 license 说明。
- WebView CSP 保持严格策略，不允许内联脚本和内联样式。Markdown 渲染结果进入 DOM 前会移除
  `<style>` 标签和 `style` 属性，避免总结内容触发 CSP 报错。
- JS 侧命令必须经 `src/ui/detail/models.py` 校验后再影响主窗口状态，避免过期 revision、
  非当前记录或 malformed payload 修改数据。
...
- 回放时根据当前播放位置在详情 WebView 中高亮对应句子和 token
- 支持导出为 SRT 字幕格式
...
- 禁止提交docs目录下的任何文件，包括superpowers子目录。
...
```bash
python -m pytest tests/test_task_manager.py tests/test_task_persistence.py tests/test_qt_main_window_ch07.py tests/test_qt_main_window_p0.py -q
```
